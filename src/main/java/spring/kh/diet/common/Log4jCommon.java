...
//		    }
		...
		/*
		 * //파일 객체 생성 File file = new File("C:\\log4j\\Sample.txt");
		 * 
		 * //쓰기 스트림 BufferedWriter bufferedWriter = new BufferedWriter(new
		 * FileWriter(file)); if(file.isFile() && file.canWrite()){ //쓰기
		 * bufferedWriter.write(log.info()); //개행문자쓰기 bufferedWriter.newLine();
		 * 
...
//	}
	...
	// 서비스 쪽 포인트컷, After
	/*
	 * @AfterReturning("afterPointCut()") public void logAfterService(JoinPoint JP)
	 * { String methodName = JP.getSignature().getName(); String shortString =
	 * JP.toShortString(); String serviceName = ""; StringTokenizer ST1 = new
	 * StringTokenizer(shortString, "."); String Token1 = ST1.nextToken();
	 * StringTokenizer ST2 = new StringTokenizer(Token1, "("); ST2.nextToken();
	 * serviceName = ST2.nextToken(); // System.out.println(totalDay);
	 * System.out.println("서비스 @ 메소트명 : "+methodName);
	 * System.out.println("서비스 @ 서비스명 : "+serviceName);
	 * System.out.println("서비스 @ 투스트링 : "+JP.toString()); // JP.tostring //
	 * execution(ArrayList
	 * spring.kh.diet.model.service.DietTipServiceImpl.getNextPreDt(int))
	 * 
	 * 
	 ...
	 * 
	 * System.out.println("서비스 @ 타겟 클래스 : "+JP.getTarget().getClass()); //class
	 * spring.kh.diet.model.service.DietTipServiceImpl }
	 */
}
