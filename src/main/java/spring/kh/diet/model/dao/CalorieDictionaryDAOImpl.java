...
*/			
	...
	public String getHealthCalPageNavi(SqlSessionTemplate session, int currentPage, int recordCountPerPage,
			int naviCountPerPage) {
		int recordTotalCount = session.selectOne("calorieDic.getHealthCalNavi");
...
*/			
			if (needPrev) // 시작이 1페이지가 아니라면!
			{
				sb.append("<a class='item' href='/healthCalorieList.diet?currentPage=" + (startNavi - 1) + "'> &lt; </a>");
			}
			...
			for(int i=startNavi;i<=endNavi;i++)
			{
				if(i==currentPage)
				{
					sb.append("<a class='active item' style='background: rgba(250, 40, 40); color:white;' href='/healthCalorieList.diet?currentPage=" + i + "'><strong>"+i+"</strong></a>");
				}
				else
				{
					sb.append("<a class='item' href='/healthCalorieList.diet?currentPage=" + i + "'> "+i+" </a>");
				}
			}
			if (needNext) // 끝 페이지가 아니라면!
			{
				sb.append("<a class='item' href='/healthCalorieList.diet?currentPage=" + (endNavi + 1) + "'> &gt; </a>");
			}
