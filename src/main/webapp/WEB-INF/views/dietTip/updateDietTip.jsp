...
<body>
	...
	<!-- CONTENTS -->
	<input id="mainPhotoPath" type="hidden" value="${requestScope.dt.dtMainPhoto }">
	<input type="hidden" id="indexNo" value="${requestScope.dt.dtIndex }">
	<br>
	<br>
		...
		<div class="ui left aligned basic segment">
			...
			<div class="ui form" style="max-width: 1220px;">
				...
				<div class="ui fluid left action icon input">
					<div class="ui basic floating dropdown button">
						<div class="text">
							<c:choose>
								<c:when test="${requestScope.dt.dtType==1 }">
									칼럼
								</c:when>
								<c:when test="${requestScope.dt.dtType==2 }">
									운동
								</c:when>
								<c:when test="${requestScope.dt.dtType==3 }">
									식단
								</c:when>
								<c:when test="${requestScope.dt.dtType==4 }">
									성공후기
								</c:when>
							</c:choose>
						</div>
					...
					<input type="text" id="title" value="${requestScope.dt.dtTitle }" />
				</div>
			...
			<div class="ui grid">
				...
				<div class="ten wide column">
					...
					<textarea style="width: 100%; height: 200px; resize: none;" id="sammary">${requestScope.dt.dtSammary }</textarea>
				</div>
			</div>
			<br>
			<div id="summernote">
				${requestScope.dt.dtExplain }
			</div>
			<br>
			<div align="center">
				<button class="ui red basic button" onclick="register();">수정 완료</button>
			</div>
		</div>
	</div>
...
function imgUpload(file, el) {
	...
	category = '${requestScope.dt.dtType }';
